...
func ImportantDebug(format string, a ...interface{}) {
	Debug(0, format, a...)
}
...
func Debug(level int, format string, a ...interface{}) {
	msg := fmt.Sprintf("ECFChecker: "+format, a...)
	switch log.Lvl(level + 2) {
	case log.LvlTrace:
		log.Trace(msg)
	case log.LvlDebug:
		log.Debug(msg)
	...
	//log.Root().write(msg, debugLevel, nil)
...
func (checker *Checker) PushNewSegmentFromStart(contract *Contract) {
	...
	if checker.runningSegments.Len() == 0 {
		segment = Segment{contract: contract.Address(),
			depth:              1,
			prevSegment:        nil,
			readSet:            set.New(set.ThreadSafe),
			writeSet:           set.New(set.ThreadSafe),
...
func findCutpoint(trace []Segment, baseDepth int) int {
	...
	for cutpoint = len(trace); cutpoint > 0; cutpoint-- { // Guessing the cutpoint
		...
		prefixReadSet := set.New(set.ThreadSafe)
		prefixWriteSet := set.New(set.ThreadSafe)
...
func (checker *Checker) UponEVMStart(evm *EVM, contract *Contract) {
	if DISABLE_CHECKER {
		return
	}
	...
	if checker.evmStack.Len() == 0 {
		checker.origin = &evm.Origin
		checker.blockNumber = evm.BlockNumber
		checker.time = evm.Time
...
func (checker *Checker) UponEVMEnd(evm *EVM, contract *Contract) {
	if DISABLE_CHECKER {
		return
	}
	...
	if checker.runningSegments.Len() == 1 && activeCallIsARealCall {
		...
		Debug(2, "Transaction ended (Block #%v, contract %v). Checking if ECF with respect to all participating contracts.", evm.BlockNumber, FirstSegment.contract.Hex())
		...
		Debug(2, "Reentrancy check (Block #%v, contract %v) took %s / %s total", evm.BlockNumber, FirstSegment.contract.Hex(), reentrancyCheckDuration, totalProcessDuration)
	}
